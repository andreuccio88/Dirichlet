...
hosp <- t(hosp)
hosp <- hosp[,1]
D$h <- log(as.numeric(hosp[2:16]))*10
...
# m0[j] ~ dnorm(0, 1/t[j]) #intercept prior
# m1[j] ~ dnorm(0, 1/t[j]) #      mat.x prior
# m2[j] ~ dnorm(0, 1/t[j])
# t[j] ~ dgamma(0.001,0.001)
